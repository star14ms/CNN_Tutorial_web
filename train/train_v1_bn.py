"""
Train Simple CNN + BatchNorm + Dropout and export ONNX layer checkpoints.

Architecture:
  Conv1(in_channels→32, 3×3) + BN + ReLU  → (32, img_size, img_size)     [layer0]
  MaxPool(2×2)                             → (32, img_size/2, img_size/2)  [layer1]
  Conv2(32→64, 3×3) + BN + ReLU           → (64, img_size/2, img_size/2)  [layer2]
...
Usage:
  ...
  python train/train_v1_bn.py --dataset cifar10
  python train/train_v1_bn.py --dataset cifar100
  python train/train_v1_bn.py --dataset svhn
"""
...
def train(dataset_id='mnist'):
    ...
    model = SimpleCNNBN(cfg['in_channels'], cfg['img_size'], cfg['num_classes']).to(device)
...
def main():
    ...
    exports = [
        (V1BNUpToLayer0(model),      'layer0.onnx', (1, 32, S,    S)),
        (V1BNUpToLayer1(model),      'layer1.onnx', (1, 32, S//2, S//2)),
        (V1BNUpToLayer2(model),      'layer2.onnx', (1, 64, S//2, S//2)),
        (V1BNUpToLayer3(model),      'layer3.onnx', (1, 64, feat, feat)),
        (V1BNUpToLayer4(model),      'layer4.onnx', (1, 128)),
        (V1BNFullWithSoftmax(model), 'full.onnx',   (1, N)),
    ]
